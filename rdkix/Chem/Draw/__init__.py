...
from rdkix.Chem.Draw.MolDrawing import DrawingOptions
from rdkix.Chem.Draw.MolDrawing import MolDrawing
...
def _getCanvas():
  ...
  useSping = False
  Canvas = None
  if not os.environ.get('RDKIX_CANVAS', ''):
    try:
      from rdkix.Chem.Draw.cairoCanvas import Canvas
      useCairo = True
    except ImportError:
      try:
        from rdkix.Chem.Draw.aggCanvas import Canvas
        useAGG = True
      except ImportError:
        from rdkix.Chem.Draw.spingCanvas import Canvas
        useSping = True
  else:
    canv = os.environ['RDKIX_CANVAS'].lower()
    if canv == 'cairo':
      from rdkix.Chem.Draw.cairoCanvas import Canvas
      useCairo = True
    elif canv == 'agg':
...
def _legacyMolToImage(mol, size, kekulize, wedgeBonds, fitImage, options, canvas, **kwargs):
  ...
  if legend:
    from rdkix.Chem.Draw.MolDrawing import Font
...
def MolToMPL(mol, size=(300, 300), kekulize=True, wedgeBonds=True, imageType=None, fitImage=False,
  ...
  if not mol:
    raise ValueError('Null molecule provided')
  from rdkix.Chem.Draw.mplCanvas import Canvas
  canvas = Canvas(size)
  if options is None:
    options = DrawingOptions()
...
def MolToQPixmap(mol, size=(300, 300), kekulize=True, wedgeBonds=True, fitImage=False, options=None,
  ...
  if not mol:
    raise ValueError('Null molecule provided')
  from rdkix.Chem.Draw.qtCanvas import Canvas
  canvas = Canvas(size)
  if options is None:
    options = DrawingOptions()
  options.bgColor = None
