...
def get_unique_products(rxn, mol):
  ...
  for newmol in rxn.RunReactants((mol, )):
    ...
    assert newmol is not None, '%s produced %s when applying' % (Chem.MolToSmiles(
      mol, isomericSmiles=True), isosmiles)
...
class TestCase(unittest.TestCase):
  ...
  def test_apply_every_rule_to_every_fragment(self):
    ...
    for row in frag_reader:
      smiles = row['SMILES']
      rdkix_mol = Chem.MolFromSmiles(smiles)
      orig_can_smi = Chem.MolToSmiles(rdkix_mol, isomericSmiles=True)

      changed = False

      for src, rxn in zip(GetHeterocycleReactionSmarts(), GetHeterocycleReactions()):
        for smiles in get_unique_products(rxn, rdkix_mol):
      ...
      if not changed and has_aromatic(rdkix_mol):
...
# [c;h1;D2;r6;$(a1ccc[c,n,o][c,n,o]1),$(a1cc[c,n,o][c,n,o]c1),$(a1c[c,n,o][c,n,o]cc1),$(a1[c,n,o][c,n,o]ccc1),$(a1cc[n,o]c[n,o]1),$(a1c[n,o]c[n,o]c1),$(a1[n,o]c[n,o]cc1),$(a1[n,o]ccc[n,o]1),$(a1c[n,o]cc[n,o]1),$(a1[n,o]cc[n,o]c1):1]
  ...
  def get_six_member_ring_carbon_to_nitrogen_reaction(self):
    ...
    rxns = [
      r for r in GetHeterocycleReactionSmarts() if r.DESCRIPTION.startswith(expected_description)
    ]
  ...
  def test_six_member_ring_carbon_to_nitrogen_should_hit_all_other_carbons(self):
    ...
    for smiles, expected_num_matches in test_inputs:
      rdkix_mol = Chem.MolFromSmiles(smiles)
      num_rdkix_matches = len(rdkix_mol.GetSubstructMatches(smarts_mol))

      assert num_rdkix_matches == expected_num_matches, "expected %u matches in %s, got %u" % (
        expected_num_matches, smiles, num_rdkix_matches)
  ...
  def test_fuzz_atom_mutations(self):
    ...
    writer.writeheader()
    notchanged = csv.DictWriter(open(base + 'not_changed_during_fuzzing.csv', 'w'),
                                ['SMILES', 'TITLE'])
    notchanged.writeheader()
    uniq_notchanged = set()
    ...
    for i in range(num_trials):
      ...
      rdkix_mol = Chem.MolFromSmiles(orig_can_smi)
  ...
  def test_reaction_on_mol_with_attachments(self):
    ...
    rdkix_mol = Chem.MolFromSmiles('c1([*:2])ccccc1([*:3])')
    products = set(get_unique_products(rxn, rdkix_mol))
    expected = set([
      'c1cnc([*:2])c([*:3])c1', 'c1cc([*:3])c([*:2])cn1', 'c1cnc([*:3])c([*:2])c1',
      'c1cc([*:2])c([*:3])cn1'
    ])
    assert products == expected, '%r != %r' % (products, expected)

    # two less of these because of symmetry
    rdkix_mol = Chem.MolFromSmiles('c1([*:2])ccc([*:3])cc1')
    assert set(get_unique_products(rxn, rdkix_mol)) == set(
      ['c1cc([*:2])ncc1[*:3]', 'c1cc([*:3])ncc1[*:2]'])

    # test what happens when nothing matches
    rdkix_mol = Chem.MolFromSmiles('c1([*:2])cc[nH]c1([*:3])')
    assert set(get_unique_products(rxn, rdkix_mol)) == set()
