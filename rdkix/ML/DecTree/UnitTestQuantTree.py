...
from rdkix.ML.DecTree.QuantTree import QuantTreeNode
from rdkix.TestRunner import redirect_stdout
...
class TestCase(unittest.TestCase):
  ...
  def test3Classify(self):
    ...
    for i in range(len(self.examples1)):
      self.assertEqual(self.t1.ClassifyExample(self.examples1[i]), self.examples1[i][-1],
                       msg='examples1[%d] misclassified' % i)
    for i in range(len(self.examples2)):
      self.assertEqual(self.t2.ClassifyExample(self.examples2[i]), self.examples2[i][-1],
