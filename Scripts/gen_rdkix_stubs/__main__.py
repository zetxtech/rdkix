...
"""
Script to generate Python stubs for RDKix.
...
make sure that the RDKix Python modules for which stubs are
to be generated are the *first* RDKix modules available in
sys.path; otherwise, stubs will not be generated for the
intended RDKix version.

Usage:
./Scripts/gen_rdkix_stubs.py [output_dirs; defaults to $PWD]

Usage example:
$ cd $RDBASE
$ ./Scripts/gen_rdkix_stubs.py
$ cp -R rdkix-stubs $CONDA_PREFIX/lib/python3.*/site-packages

The scripts creates an rdkix-stubs directory in each
...
from . import generate_stubs, purge_rdkix_source_dir_from_sys_path, find_rdkix_site_packages_path
...
if __name__ == "__main__":
    ...
    site_packages_path = find_rdkix_site_packages_path()
    try:
