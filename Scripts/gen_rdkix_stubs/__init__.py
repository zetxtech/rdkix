...
RDKIX_MODULE_NAME = "rdkix"
RDBASE_MODULE_NAME = "rdBase"
INIT_PY = "__init__.py"

def rdkix_has_rdbase(rdkix_dir):
    """Returns True if rdkix_dir contains the rdBase Python module.

    Args:
        rdkix_dir (str): directory path

    Returns:
        bool: True if rdkix_dir contains the rdBase Python module
    """
    return any(f.startswith(RDBASE_MODULE_NAME) for f in os.listdir(rdkix_dir))

def purge_rdkix_source_dir_from_sys_path():
    """Remove the rdkix source dir from sys.path if present."""
    if os.path.isdir(RDKIX_MODULE_NAME) and not rdkix_has_rdbase(RDKIX_MODULE_NAME):
        abs_cwd = os.path.abspath(os.getcwd())
        indices_to_pop = sorted([i for i, p in enumerate(sys.path) if os.path.abspath(p) == abs_cwd], reverse=True)
        for i in indices_to_pop:
            sys.path.pop(i)

def find_rdkix_site_packages_path():
    site_packages_path = None
    for path_entry in sys.path:
        if not path_entry:
            continue
        rdkix_path = os.path.join(path_entry, RDKIX_MODULE_NAME)
        if os.path.isdir(rdkix_path) and rdkix_has_rdbase(rdkix_path):
            site_packages_path = path_entry
            break
    if site_packages_path is None:
        raise ValueError("Failed to find rdkix in PYTHONPATH")
    return site_packages_path

def find_rdkix_include_path():
    rdkix_include_path = None
    try:
        rdkix_include_path = find_rdkix_site_packages_path()
    except ValueError:
        pass
    if rdkix_include_path is not None:
        for i in range(3):
            rdkix_include_path = os.path.dirname(rdkix_include_path)
        rdkix_include_path = os.path.join(rdkix_include_path, "include")
        if not os.path.isdir(os.path.join(rdkix_include_path, RDKIX_MODULE_NAME)):
            rdkix_include_path = None
    return rdkix_include_path
...
def clear_stubs(outer_dir):
    """Remove all files and directories from "rdkix-stubs"
...
def generate_stubs_internal(modules, outer_dirs, args):
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        src_dir = os.path.join(tempdir, RDKIX_MODULE_NAME)
...
def generate_stubs(site_packages_path, args):
    """Generate RDKix stubs.

    Args:
        site_packages_path (str): full path from where RDKix modules are
        imported.
        output_dirs (list, optional): List of directories where a rdkix-stubs.
    ...
    modules = {str(p.parent.relative_to(site_packages_path)).replace(os.sep, ".")
               for p in sorted(site_packages_path.joinpath(RDKIX_MODULE_NAME).rglob(INIT_PY))}
    outer_dirs = []
    for output_dir in output_dirs:
        outer_dir = os.path.join(output_dir, f"{RDKIX_MODULE_NAME}-stubs")
    ...
    modules = {str(p.relative_to(site_packages_path.joinpath(RDKIX_MODULE_NAME)))
               for p in (
        sorted(site_packages_path.joinpath(RDKIX_MODULE_NAME).rglob("*.pyd")) +
        sorted(site_packages_path.joinpath(RDKIX_MODULE_NAME).rglob("*.so")) +
        sorted(site_packages_path.joinpath(RDKIX_MODULE_NAME).rglob("*.py"))
    ) if p.name != INIT_PY}.difference(pyi_files)
    modules = {RDKIX_MODULE_NAME + "." + os.path.splitext(p.replace(os.sep, "."))[0] for p in modules}
    generate_stubs_internal(modules, outer_dirs, args)
...
class ProcessDocLines:
    ...
    CPP_PYTHONIC_RETURN_TYPES = {
        ...
        "_ROConformerSeq": "typing.Sequence[rdkix.Chem.Conformer]",
        "_ROQAtomSeq": "typing.Sequence[rdkix.Chem.QueryAtom]",
        "_vectd": "typing.Sequence[double]",
        "_vecti": "typing.Sequence[int]",
        "_vectj": "typing.Sequence[int]",
        "_vectN5RDKix13Abbreviations22AbbreviationDefinitionE": "rdkix.Chem.rdAbbreviations.AbbreviationDefinition]",
        "_vectN5RDKix9Chirality10StereoInfoE": "typing.Sequence[rdkix.Chem.StereoInfo]",
    ...
    def convert_to_valid_type(self, py_signature_ret):
        ...
        or an RDKix class type. Builtins are returned unchanged, while
        RDKix class types are prefixed with the appropriate module such
