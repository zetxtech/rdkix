...
"""
This module will scan RDKix sources searching for docstrings
...
class FixSignatures:
    ...
    rdkix_stubs_path = os.path.join(os.getcwd(), f"{RDKIX_MODULE_NAME}-stubs")
    clean = False
    ...
    def __init__(self, args=None):
        ...
        if self.rdkix_include_path is None:
            print(f"Failed to find RDKix include path. Please set {self.__class__.__name__}.rdkix_include_path")
            sys.exit(1)
    ...
    def get_rdk_build_flags(self):
        """Generate command line clang++ flags to build RDKix
        based on the contents of RDGeneral/RDConfig.h.

        Returns:
            str: command line clang++ flags
        """
        rdconfig_h = os.path.join(self.rdkix_include_path, RDKIX_MODULE_NAME, "RDGeneral", "RDConfig.h")
        definitions = set()
        includes = set()
    ...
    def init_clang_worker_data(self):
        ...
        rdkix_stubs_path = Path(self.rdkix_stubs_path)
        python_include_path = f"-I{self.python_include_path}" if self.python_include_path else ""
        rdkix_code = os.path.join(self.cpp_source_path, "Code")
        rdkix_external = os.path.join(self.cpp_source_path, "External")
        ...
        rdkix_external_path = Path(rdkix_external)
        avalon_include_dir = os.path.abspath(str(max(rdkix_external_path.rglob("AvalonTools/ava-formake-AvalonToolkit_*/src/main/C/include"))))

        clang_flags = (
            f"-I{self.include_path} {python_include_path} -I{rdkix_code} "
            f"-I{rdkix_external} -I{avalon_include_dir} -I. -I..{qt_include_dirs}"
            f" {rdk_build_defs} {self.clang_flags}{user_clang_flags}"
        ).strip().split()
        self.clang_worker_data = ClangWorkerData(clang_flags)
        for p in sorted(rdkix_stubs_path.rglob("*.pyi")):
            if str(p.stem) == "__init__":
                p = p.parent
            pyi_module_path = os.path.splitext(str(p.relative_to(rdkix_stubs_path)).replace("/", "."))[0]
            if pyi_module_path == ".":
                pyi_module_path = RDKIX_MODULE_NAME
            else:
                pyi_module_path = RDKIX_MODULE_NAME + "." + pyi_module_path
            try:
