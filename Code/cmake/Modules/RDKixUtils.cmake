...
# Mac OS X specific code
  set(RDKix_VERSION "${RDKix_Year}.${RDKix_Month}")
ELSE(${CMAKE_SYSTEM_NAME} MATCHES "Darwin")
  set(RDKix_VERSION "${RDKix_ABI}.${RDKix_Year}.${RDKix_Month}")
ENDIF(${CMAKE_SYSTEM_NAME} MATCHES "Darwin")
set(RDKix_VERSION "${RDKix_VERSION}.${RDKix_Revision}${RDKix_RevisionModifier}")
set(RDKix_RELEASENAME "${RDKix_Year}.${RDKix_Month}.${RDKix_Revision}${RDKix_RevisionModifier}")
...
set(RDKix_BUILDNAME "${CMAKE_SYSTEM_NAME}|${CMAKE_SYSTEM_VERSION}|${systemAttribute}|${compilerID}|${bit3264}")
set(RDKix_EXPORTED_TARGETS rdkix-targets)
...
macro(rdkix_library)
  ...
  if((MSVC AND (NOT RDK_INSTALL_DLLS_MSVC)) OR (WIN32 AND RDK_INSTALL_STATIC_LIBS))
    ...
    target_link_libraries(${RDKLIB_NAME} PUBLIC rdkix_base)
    if(RDK_INSTALL_DEV_COMPONENT)
      INSTALL(TARGETS ${RDKLIB_NAME} EXPORT ${RDKix_EXPORTED_TARGETS}
    ...
    if(RDK_INSTALL_STATIC_LIBS)
      ...
      endforeach()
      target_link_libraries(${RDKLIB_NAME}_static PUBLIC rdkix_base)
      if(RDK_INSTALL_DEV_COMPONENT)
        INSTALL(TARGETS ${RDKLIB_NAME}_static EXPORT ${RDKix_EXPORTED_TARGETS}
                DESTINATION ${RDKix_LibDir}/${RDKLIB_DEST}
                COMPONENT dev )
      endif(RDK_INSTALL_DEV_COMPONENT)
      set_target_properties(${RDKLIB_NAME}_static PROPERTIES
                            OUTPUT_NAME "RDKix${RDKLIB_NAME}_static")
  ...
  if (NOT RDK_INSTALL_INTREE)
    ...
    if(RDK_INSTALL_DEV_COMPONENT)
      install(FILES ${RDKHDR_DEFAULT_ARGS}
              DESTINATION ${RDKix_HdrDir}/${RDKHDR_DEST}
              COMPONENT dev )
    endif(RDK_INSTALL_DEV_COMPONENT)
  endif(NOT RDK_INSTALL_INTREE)
endmacro(rdkix_headers)

macro(rdkix_python_extension)
  ...
  if(RDK_BUILD_PYTHON_WRAPPERS)
    ...
    target_link_libraries(${RDKPY_NAME} PUBLIC ${RDKPY_LINK_LIBRARIES}
                          RDBoost rdkix_py_base rdkix_base )
    ...
    INSTALL(TARGETS ${RDKPY_NAME}
            LIBRARY DESTINATION ${RDKix_PythonDir}/${RDKPY_DEST} COMPONENT python)
  endif(RDK_BUILD_PYTHON_WRAPPERS)
endmacro(rdkix_python_extension)
...
macro(rdkix_catch_test)
  ...
  if(RDK_BUILD_CPP_TESTS)
    add_executable(${RDKTEST_NAME} ${RDKTEST_SOURCES})
    target_link_libraries(${RDKTEST_NAME} PRIVATE rdkixCatch ${RDKTEST_LINK_LIBRARIES} Catch2::Catch2)
    add_test(${RDKTEST_NAME} ${EXECUTABLE_OUTPUT_PATH}/${RDKTEST_NAME})
  endif(RDK_BUILD_CPP_TESTS)
endmacro(rdkix_catch_test)
...
macro(add_pytest)
  ...
  if(RDK_BUILD_PYTHON_WRAPPERS)
    add_test(${PYTEST_NAME}  ${Python3_EXECUTABLE}
             ${PYTEST_SOURCES})
    SET(RDKIX_PYTEST_CACHE "${PYTEST_NAME};${RDKIX_PYTEST_CACHE}" CACHE INTERNAL "Global list of python tests")
  endif(RDK_BUILD_PYTHON_WRAPPERS)
endmacro(add_pytest)
...
function(add_jupytertest testname workingdir notebook)
  if(RDK_BUILD_PYTHON_WRAPPERS AND RDK_NBVAL_AVAILABLE)
    ...
    SET(RDKIX_JUPYTERTEST_CACHE "${testname};${RDKIX_JUPYTERTEST_CACHE}" CACHE INTERNAL "Global list of jupyter tests")
  endif()
...
function(add_pythonpytest testname workingdir)
  if(RDK_BUILD_PYTHON_WRAPPERS)
    add_test(NAME ${testname}  COMMAND ${Python3_EXECUTABLE} -m pytest 
       WORKING_DIRECTORY ${workingdir} )
    SET(RDKIX_PYTHONTEST_CACHE "${testname};${RDKIX_PYTHONTEST_CACHE}" CACHE INTERNAL "Global list of pytest tests")
  endif()
...
function(createExportTestHeaders)
  ...
  foreach(cmakeList ${cmakeLists})
    file(STRINGS ${cmakeList} rdkixLibraryItems REGEX "rdkix_library[ ]*\\([ ]*[^ ]+.*$")
    if (NOT "${rdkixLibraryItems}" STREQUAL "")
      foreach (rdkixLibrary ${rdkixLibraryItems})
        string(REGEX REPLACE "^[ ]*rdkix_library[ ]*\\([ ]*([^ ]+).*$" "\\1" libName "${rdkixLibrary}")
  ...
  foreach(exportLib ${exportLibs})
    ...
    file(APPEND "${CMAKE_BINARY_DIR}/${exportPath}.tmp"
      "\n"
      "// RDKIX_${exportLib}_EXPORT definitions\n"
      "#ifdef RDKIX_${exportLib}_BUILD\n"
      "#define RDKIX_${exportLib}_EXPORT RDKIX_EXPORT_API\n"
      "#else\n"
      "#define RDKIX_${exportLib}_EXPORT RDKIX_IMPORT_API\n"
      "#endif\n"
      "// RDKIX_${exportLib}_EXPORT end definitions\n")
    file(APPEND "${CMAKE_BINARY_DIR}/${testPath}.tmp"
      "\n"
      "#ifdef RDKIX_${exportLib}_BUILD\n"
      "#undef RDKIX_${exportLib}_BUILD\n"
      "#endif\n")
  endforeach()
  file(APPEND "${CMAKE_BINARY_DIR}/${exportPath}.tmp"
  "\n"
  ...
  " */\n"
  "// RDKIX_QUERY_EXPORT definitions\n"
  "#if defined(RDKIX_DYN_LINK) && defined(WIN32) && defined(BOOST_HAS_DECLSPEC)\n"
  "#define RDKIX_QUERY_EXPORT\n"
  "#else\n"
  "#define RDKIX_QUERY_EXPORT RDKIX_GRAPHMOL_EXPORT\n"
  "#endif\n"
  "// RDKIX_QUERY_EXPORT end definitions\n")
...
function(patchCoordGenMaeExportHeaders keyword path)
  file(APPEND "${path}"
    ...
    "#if !defined(RDKIX_DYN_LINK) || defined(SWIG)\n"
