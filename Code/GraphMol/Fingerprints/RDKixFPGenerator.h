...
class RDKIX_FINGERPRINTS_EXPORT RDKixFPArguments : public FingerprintArguments {
 public:
  ...
  /**
   \brief Construct a new RDKixFPArguments object
   ...
   */
  RDKixFPArguments(unsigned int minPath, unsigned int maxPath, bool useHs,
                   bool branchedPaths, bool useBondOrder, bool countSimulation,
                   const std::vector<std::uint32_t> countBounds,
                   std::uint32_t fpSize, std::uint32_t numBitsPerFeature);
};

class RDKIX_FINGERPRINTS_EXPORT RDKixFPAtomInvGenerator
 ...
 public:
  ...
  std::string infoString() const override;
  RDKixFPAtomInvGenerator *clone() const override;
};

template <typename OutputType>
class RDKIX_FINGERPRINTS_EXPORT RDKixFPAtomEnv
 ...
 public:
  ...
  /**
  \brief Construct a new RDKixFPAtomEnv object
  ...
  */
  RDKixFPAtomEnv(const OutputType bitId, boost::dynamic_bitset<> atomsInPath,
                 INT_VECT bondPath)
      : d_bitId(bitId),
...
template <typename OutputType>
class RDKIX_FINGERPRINTS_EXPORT RDKixFPEnvGenerator
...
/**
 \brief Get a RDKix fingerprint generator with given parameters
 ...
 /return FingerprintGenerator<OutputType>* that generates RDKix fingerprints
 ...
 */
template <typename OutputType>
RDKIX_FINGERPRINTS_EXPORT FingerprintGenerator<OutputType> *getRDKixFPGenerator(
