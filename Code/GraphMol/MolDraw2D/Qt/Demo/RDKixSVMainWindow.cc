...
#include "RDKixSVPanel.H"
#include "RDKixSVSettings.H"
...
// ****************************************************************************
RDKixSVMainWindow::RDKixSVMainWindow(int argc, char **argv) : QMainWindow() {
  build_actions();
  build_menubar();
  build_widget();
...
// ****************************************************************************
void RDKixSVMainWindow::build_widget() {
  left_panel_ = new RDKixSVPanel;
  right_panel_ = new RDKixSVPanel(false);
...
// ****************************************************************************
void RDKixSVMainWindow::parse_args(int argc, char **argv) {
  RDKixSVSettings s(argc, argv);
...
// ****************************************************************************
RDKixSVMainWindow::FILE_TYPE RDKixSVMainWindow::get_filetype(
...
// ****************************************************************************
void RDKixSVMainWindow::read_mols(const string &filename) {
  ...
  vector<RDKix::ROMOL_SPTR> new_mols;
  if (SDF == file_type) {
    RDKix::MolSupplier *mol_supplier =
        new RDKix::ForwardSDMolSupplier(&ins, true);
    while (!mol_supplier->atEnd()) {
      new_mols.push_back(RDKix::ROMOL_SPTR(mol_supplier->next()));
      if (!new_mols.back()) {
        new_mols.pop_back();
        break;
      }
    ...
    while (isb != eos) {
      ...
      if ('#' != nextline[0]) {
        ...
        boost::trim(smi);
        boost::trim(smi_name);
        RDKix::ROMol *mol = RDKix::SmilesToMol(smi);
        if (mol) {
          mol->setProp("_Name", smi_name);
          mols_.push_back(RDKix::ROMOL_SPTR(mol));
        }
      }
      ++isb;  // get past '\n'
    }
...
// ****************************************************************************
void RDKixSVMainWindow::read_smarts(const string &filename) {
#ifdef NOTYET
...
// ****************************************************************************
void RDKixSVMainWindow::write_mols(RDKixSVPanel &panel,
                                   const string &filename) {
  vector<RDKix::ROMOL_SPTR> mols = panel.get_molecules();
  if (mols.empty()) {
  ...
  if (SMILES == file_type) {
    bool ism = filename.substr(filename.length() - 4) == ".ism" ||
                   ...
                   ? true
                   : false;
    mw = new RDKix::SmilesWriter(&ons, " ", "Name", true, false, ism);
  } else if (SDF == file_type) {
    mw = new RDKix::SDWriter(&ons, false);
  } else {
  ...
  BOOST_FOREACH (RDKix::ROMOL_SPTR mol, mols) { mw->write(*mol); }

  delete mw;
}

// ****************************************************************************
void RDKixSVMainWindow::match_smarts(const vector<pair<string, string>> &smts) {
  ...
  for (int i = 0, is = smts.size(); i < is; ++i) {
    ...
    RDKix::RWMol *q = RDKix::SmartsToMol(smts[i].second);
    for (int j = 0, js = mols_.size(); j < js; ++j) {
      vector<RDKix::MatchVectType> hits_vect;
      if (RDKix::SubstructMatch(*mols_[j], *q, hits_vect)) {
        // store the numbers of the atoms that matched
        typedef pair<int, int> INTPAIR;
        BOOST_FOREACH (RDKix::MatchVectType hit, hits_vect) {
  ...
  vector<RDKix::ROMOL_SPTR> match_mols, miss_mols;
...
// ****************************************************************************
vector<pair<string, string>> RDKixSVMainWindow::select_smarts(
    bool multi_select) {
...
// ****************************************************************************
void RDKixSVMainWindow::update_smarts(const string &new_name,
...
// ****************************************************************************
void RDKixSVMainWindow::slot_read_mols() {
...
// ****************************************************************************
void RDKixSVMainWindow::slot_read_smarts() {
  QString filename =
...
// ****************************************************************************
void RDKixSVMainWindow::slot_exit() { exit(0); }

// ****************************************************************************
void RDKixSVMainWindow::slot_match_smarts() {
...
// ****************************************************************************
void RDKixSVMainWindow::slot_edit_smarts() {
  if (smarts_.empty()) {
    slot_new_smarts();
  }
...
// ****************************************************************************
void RDKixSVMainWindow::slot_new_smarts() {
...
// ****************************************************************************
void RDKixSVMainWindow::slot_write_left_molecules() {
...
// ****************************************************************************
void RDKixSVMainWindow::slot_write_right_molecules() {
...
// ****************************************************************************
void RDKixSVMainWindow::slot_write_smarts() {
  QString filename =
