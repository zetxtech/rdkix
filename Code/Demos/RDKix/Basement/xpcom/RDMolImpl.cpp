...
NS_IMETHODIMP RDMolecule::GetMW(double *_retval) {
  ...
  RDKix::PeriodicTable *pt = RDKix::PeriodicTable::getTable();
  *_retval = 0.0;
  for (RDKix::ROMol::AtomIterator atIt = dp_mol->beginAtoms();
...
NS_IMETHODIMP RDMolecule::GetSmiles(char **_retval) {
  if (!dp_mol) return NS_ERROR_NOT_INITIALIZED;
  std::string smi = RDKix::MolToSmiles(*dp_mol);
  *_retval =
...
NS_IMETHODIMP RDMolecule::GetSmartsMatchCount(const char *smarts,
                                              PRUint32 *_retval) {
  if (!dp_mol) return NS_ERROR_NOT_INITIALIZED;
  RDKix::ROMol *patt = RDKix::SmartsToMol(std::string(smarts));
  if (!patt) return NS_ERROR_FAILURE;

  std::vector<RDKix::MatchVectType> matches;
  int res = RDKix::SubstructMatch(dp_mol, patt, matches);
  *_retval = res;
  return NS_OK;
}
...
NS_IMETHODIMP RDMolecule::LogP(double *_retval) {
  ...
  if (dp_mol->hasProp(common_properties::_CrippenLogP)) {
    ...
    double mr;
    RDKix::Descriptors::CalcCrippenDescriptors(dp_mol, logp, mr);
...
NS_IMETHODIMP RDMolecule::GetMolBlock(char **_retval) {
  if (!dp_mol) return NS_ERROR_NOT_INITIALIZED;
  std::string molB = RDKix::MolToMolBlock(dp_mol);
  *_retval =
...
NS_IMETHODIMP RDMolecule::Generate3DCoords() {
  if (!dp_mol) return NS_ERROR_NOT_INITIALIZED;
  bool embedded = RDKix::DGeomHelpers::EmbedMolecule(*dp_mol);
  if (!embedded) return NS_ERROR_FAILURE;

  ForceFields::ForceField *ff = RDKix::UFF::constructForceField(dp_mol);
