...
std::string MolToSVG(const ROMol &mol) {
  std::vector<int> drawing = RDKix::Drawing::MolToDrawing(mol);
  std::string svg = RDKix::Drawing::DrawingToSVG(drawing);
  return svg;
}
...
void MolToCairo(const ROMol &mol, cairo_t *cr, int width, int height) {
  ...
  std::vector<int> drawing = RDKix::Drawing::MolToDrawing(mol);
  RDKix::Drawing::DrawingToCairo(drawing, cr, width, height);
}
#endif
